...
### Changed — 2026-07-14 · CHANGELOG/ROADMAP are local-only
- `CHANGELOG.md` and `ROADMAP.md` are no longer part of the public GitHub repo
  (removed and gitignored) — only `README.md` is published there going
  forward. Removed the now-dead `CHANGELOG.md` links from the README, and
  simplified `.gitignore`'s comments so they don't describe internal workflow
  details on the public repo.
...
- Added an **External** card to the dashboard (last card, right beside
  **Loopback**), mirroring the IPv4/IPv6 and TCP/UDP two-way switch cards.
  Resolves the pending decision from 2026-07-06 about the Scope section.
...
- Bundle id changed to **`com.jensyleo.tcpv4mac`** (matching the identifier style
  used for HG4MAC), so the app carries no employer or organization association.
